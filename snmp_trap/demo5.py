...
def cbFun(snmpEngine, stateReference, contextEngineId, contextName,
    ...
    # 只选取特定的字段
    selected_info = {
        "告警产生时间": trap_info.get("告警产生时间"),
        "告警级别": trap_info.get("告警级别"),
        "设备类型": trap_info.get("设备类型"),
        "告警序列号": trap_info.get("告警序列号"),
        "清除告警流水": trap_info.get("清除告警流水"),
        "告警名称": trap_info.get("告警名称"),
        "子架名称": trap_info.get("子架名称"),
        "单板名称": trap_info.get("单板名称"),
        "是否是清除告警": trap_info.get("是否是清除告警")
    }

    # 转换为 JSON
    selected_json_data = json.dumps(selected_info, ensure_ascii=False)

    # 发送邮件
    #subject = "Trap通知"
    #email_body = f"接收到新的Trap消息:\n{json_data}"
    #send_email(subject, email_body)
    # 转换 JSON 为 HTML 表格
    html_table = json_to_html_table(selected_json_data)
    # 发送邮件
    subject = "Trap通知"
